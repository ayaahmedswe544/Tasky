...
namespace Tasky.Repositories.Repos
{
    public class AccountRepo : IAccountRepo
    {
        ...
        public AccountRepo(UserManager<AppUser> userManager, SignInManager<AppUser> signInManager, RoleManager<IdentityRole> roleManager)
        {
        ...
        public async Task<bool> UpdateUserAsync(UpdateVM UserVM)
        {
            var email = _signInManager.Context.User?.FindFirst(ClaimTypes.Email)?.Value;

            if (string.IsNullOrEmpty(email))
            {
                Console.WriteLine("No user email found in claims.");
                return false;
            }
            ...
            user.FullName = UserVM.FullName ?? user.FullName;
